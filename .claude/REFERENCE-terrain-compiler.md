...
**Day-hikes (37 activity points)** — `_build/geocode_dayhikes.mjs` resolves coords via **Photon** (state-bbox guard, no prior coord to anchor on) + a hand-filled `COORD` map for the slot canyons Photon can't index; rendered as **green up-triangles** (distinct from stay-diamonds + camp-circles). The first member of a future "activities" layer (restaurants, viewpoints, swimming holes).
