...
**Day-hikes (37 in-park marquee + 124 standalone = 161 activity points)** — the original 37 (slot canyons, via-ferrata, urban high-lines) were geocoded via **Photon** (`_build/geocode_dayhikes.mjs`, state-bbox guard + a hand-filled `COORD` map for the slots Photon can't index); the **Wave-2 standalone sweep** (below) added 124 more. Rendered as **green up-triangles** (distinct from stay-diamonds + camp-circles). The first member of a future "activities" layer (restaurants, viewpoints, swimming holes).

**The Wave-2 standalone sweep (the "what did we miss off-system?" pass).** The 37 in-park day-hikes are attached to units the atlas already enumerates (Zion, Yosemite). The open question: *which notable day-hikes and trail networks sit on land we DON'T cover* — city/county/regional-district/private/nonprofit/university land, **not** inside a National/State Park, NF, BLM, NWR, or WMA. That's the standalone set, and it's the genuinely-additive one. Method: **11 agents** (7 regional day-hike + 4 network — 2 MTB, 2 open-space) each required to **state the land manager per entry** so the filter is mechanically verifiable. Consolidation (`_build/_consolidate_wave2.mjs`): drop any whose manager is a state/federal system (State Park/Forest, NF, BLM, NWR, WMA — plus the edge cases Forever Wild, TVA, USACE that other layers already touch) or an agent EXCLUDE flag; **dedup cross-kind** (a place that's a *network* must not also be a day-hike — Corner Canyon, South Mountain, Barton Creek collapse to the network) and against the existing 37; validate centroid-in-declared-state (0 out-of-state). Result: **124 standalone day-hikes + 66 trail networks** (MTB destinations like Bentonville / Kingdom Trails / Galbraith; open-space systems like Boulder OSMP / East Bay Regional / McDowell Sonoran). **Networks are a new point primitive** — a labeled *hub* (violet square) at the trailhead, with the actual web of trails supplied by the zoom-gated Waymarked substrate; `trail_miles` + land manager carried on the point. **9 borderline state-trust/utility entries** (WA DNR NRCAs like Mt Si / Mailbox, Marin Municipal Water District, Seattle Public Utilities) are kept but flagged in the consolidation report — a published-floor-style "honest tail" the user can prune. Lesson: the standalone filter is only as good as the *land-manager* field — forcing agents to name the owner turns "is this standalone?" from a judgment call into a string match.
...
**Standings:** **432 journeys** across three modes — **204 hiking** (126 exact OSM/cycling geometry + 78 corridors) + **119 biking** (48 OSM + 71 corridors) + **109 paddling** (all corridors) — **+ 161 day-hikes** (37 in-park marquee + 124 standalone) **+ 66 standalone trail networks**, joining 6,414 campgrounds + 948 vehicle routes + 884 stays. All 432 journeys + 124 standalone day-hikes + 66 networks verified centroid-in-declared-state. Committed reproducibility snapshots in `layers/{hike,bike,paddle}_journeys_meta.json` + `layers/{hike,bike,paddle}_corridors.json` (journeys) and `layers/standalone_day_hikes_meta.json` + `layers/networks_meta.json` (Wave-2); the three `build_*_journeys.mjs` read working staging with a committed-layers fallback. The atlas resolves into **four ontological tiers — regions (ecoregions) · routes (drives + journeys, all four modes) · points (camps, stays, day-hikes, network hubs) · environments (the zoom-in substrate)**. The trails-by-mode family (§13 vision) is complete and comprehensive, and the **Wave-2 standalone sweep is DONE** (124 day-hikes + 66 networks, headless-verified, dashboard `b6e917a`). Next: thread the layers (proximity join routes↔camps↔stays↔activities↔networks into a trip-planner), and the broader "activities" point-layer beyond day-hikes (restaurants, viewpoints, swimming holes).
...
| Day-hikes — standalone (124) | regional agent sweep, **land-manager-filtered** to city/county/private/nonprofit/university land (not in any enumerated park/forest/BLM unit) | point |
| Trail networks — standalone (66) | MTB + open-space agent sweep, same land-manager filter; rendered as labeled hubs over the substrate | hub point |
