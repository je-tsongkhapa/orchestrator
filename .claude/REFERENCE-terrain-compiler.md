...
Lives in `superorganism/dashboard/prototypes/`. **COMPLETE — all 12 layers built nationally: 6,541 campgrounds across 50 states, 624 fives.** Plus a **Vehicle Trails** route overlay — **948 routes** across four tiers (scenic byways, book drives, dirt routes) as road-following lines (§13) — a **Unique Stays** lodging layer — **1,104 bookable one-of-a-kind places** as points (§14) — an **Activities** "things-to-do" layer — **2,805 points** across 6 families (§17) — and the **human-powered Trails family** — **435 journeys** across hike/bike/paddle (lines) + **225 day-hikes** + **259 standalone trail networks** (activity/hub points) + a zoom-gated Waymarked "every trail" substrate (§15). **Every point layer has been coverage-audited to an even per-state 80/20** (stays, activities, day-hikes, networks). The explorer clusters the dense point layers (markercluster) + viewport-gates its list panels so it stays responsive at ~12k features. The atlas resolves into four tiers: **regions · routes · points · environments**.
...
**9 sub-types** (one enumeration agent each): **fire lookouts** (69 — recreation.gov, the one semi-authoritative source) · **historic grand hotels** (78 — Historic Hotels of America + grand dames) · **park lodges** (51 — Old Faithful Inn / Ahwahnee / El Tovar class) · **quirky rentals** (66 — treehouse/dome/yurt/A-frame/glass) · **converted-structure stays** (63 — silo/church/caboose/plane/boat/cave/missile-silo) · **roadside Americana** (44 — Wigwam Motels, Madonna Inn, Route 66 neon) · **lighthouses + water stays** (47+9) · **haunted/legendary** (52 — Stanley, Crescent, Queen Mary) · **glamping/unique resorts** (54 — Under Canvas, AutoCamp). A **completeness pass** then added the genres the first pass missed entirely: **guest/dude ranches** (18), **historic country inns** (15 — Mohonk, The Point, Twin Farms), **backcountry huts** (13 — AMC + 10th Mountain Division), **monastery/retreats** (8 — Tassajara, Gethsemani), **castle stays** (6), **farm/vineyard stays** (6), plus **floating cabins · train-car hotels · observatory · aurora-igloo · tipi/hogan** — **21 sub-types** in all. (The first completeness pass was authored inline after the agents hit the weekly limit; once the limit reset a **depth gap-fill** added ~120 more via agents — each reading the live list and adding only what was missing — deepening glamping→89, quirky→99, converted→87, roadside→67. A final pass deepened the inline-authored genres the same way — ranches→51, huts→45, country inns→38, retreats→26, farm→23, castle→14, plus the thin floating/train/observatory/tipi/ice — for **884** total.)

**Process:** enumerate per sub-type (agents, with accurate lat/lng + a 2–5 uniqueness score + why/notes/booking) → `_build/merge_stays.mjs` dedups cross-subtype overlaps (Queen Mary was in haunted *and* lighthouse; the famously-haunted historic hotels in both) by normalized name, keeping the higher score → validate coordinates in US bounds. **Strictly bookable-only** — a `merge_stays` DROP set purges anything that isn't a real overnight stay (restaurants like the Olde Pink House, tour-only sites like the Goldfield Hotel, the closed Oregon Caves Chateau, a private-residence 727). After dedup + purge + completeness + depth gap-fill: **884 stays, 50 states**; honest scores **162 fives / 461 fours / 261 threes** (skewed high because the set is pre-curated to marquee, like the dirt routes). Coordinates verified on a national scatter + a per-state bounds check — fire lookouts cluster the mountain West, roadside motels trace Route 66, lighthouses line the coasts + Great Lakes; nothing in the ocean or the wrong state.

**Coverage audit — 884 → 1,104, every state ≥9 stays (DONE).** A state×sub-type census exposed the same regional-fan-out unevenness Activities had: AL/MS/WV=1, NE=0 (missing), DE/ND/RI/DC=2-3 — under-coverage, not honest absence. Per-state gap agents (handed each state's existing stays to return only NET-NEW, constrained to the 21 known sub-types, agent coords verified bookable + in-state) filled the 18 states with ≤8 stays: NE 0→17 · AL/MS/WV/DE/DC 1-3→14-19 · the rest up to 12-21. **220 net-new; min state 0→9, median 17.** Agents held the bar — dropped closed/destroyed/not-yet-open (Pioneer Inn lost in the 2023 Lahaina fire, Savage River Lodge, Madewood), caught cross-state mislabels, honestly skipped absent sub-types. Hardened `merge_stays.mjs` with the per-state **SB in-state bbox guard** (replacing a coarse US-bounds check) — which immediately caught a stray Virgin Islands entry outside the 50-state+DC scope. Dashboard commit `de3cb5e` (local).

**Coordinate refinement (geocoding pass):** enumeration agents pin most stays to *town-center* accuracy, not the building. `_build/geocode_stays.mjs` refines them against the **Photon (OSM) geocoder** — but the trap is fuzzy matching, which bridges *different* properties through a single shared geographic or structure word ("Zion Lodge"→Zion Canyon Lodge, "Palace Hotel"→Palace of Fine Arts, "Zapata Ranch"→Zapata Falls Campground, "Castle Hot Springs"→a *fire station* 18 mi off). Three guards keep refinements honest: the matched OSM object must be **lodging-class** (hotel/hut/lighthouse/ranch/camp_site/… — never fire_station/restaurant/university/nature_reserve), its name must **share a distinctive token** with the stay (place words like "Denver"/"Orleans" stripped), and it must sit **within 15 mi** of the prior coord. A pin only moves when all three pass; everything else keeps its area coord — *an approximate pin beats a confidently-wrong one* (the same honesty rule as §5a curation). That still leaves a thin tail of single-shared-token cross-property hits no lexical rule can separate, so a hand-audited **`NAME_DROP`** set removes them (19 of an initial 239). Result: **220 of 884** stays refined (159 moved <1 mi), zero relocations to a wrong place; verified headless — refined pins frame exactly at the property.
...
## 15. Hiking Journeys + Day-Hikes — the human-powered trails family

The foot member of the trails-by-mode family (§13 built the driving member). The breakthrough was an **ontology**, not a size: the atlas reduces to **two travel primitives** — the **route** (a line) and the **activity** (a point) — over the ecoregion backdrop, with a **zoom-gated comprehensive substrate** underneath. Journeys are *sparse-curated routes shown wide*; the substrate is *dense-raw routes shown close*; same `path` primitive, different curation + zoom-gate. (Day-hikes are the *activity* primitive — a hike you slot into a day, like a stay or a meal, so a point not a line.)

**Hiking journeys (204 curated routes)** — `subtype` ∈ National-Scenic / long-distance / park-traverse / historic / weird / one-of-a-kind. The **geometry engine is the Waymarked Trails API** (`{theme}.waymarkedtrails.org/api/v1`, OSM-derived/ODbL): `list/search?query=` → relation id → `details/relation/{id}` carries the full way geometry in **Web Mercator** (decode `lat=(π/2−2·atan(e^(−y/R)))·180/π`, `lon=x/R·180/π`) + `official_length`. Recurse the `route` tree collecting every `LineString`, RDP-simplify (ε=0.0008), emit a multi-line `path` — same schema as the vehicle routes. JMT validated it (<1% length drift, exact High-Sierra bbox).

**The recurring resolution traps** (geometry was the crux a *fourth* time): (1) **super-relations** — the AT/Florida/Colorado parent ranks *below* its own state sections in name search → pick the parent by **biggest bbox lat-span** (AT = rel 156553, GA→ME); (2) **name-leak abroad** — "Presidential Traverse" matched a route at lng +74.9 (Asia) → reject any geometry whose centroid leaves US bounds; (3) **multi-relation flips** — same-named fragments resolve non-deterministically → `ID_OVERRIDE` + an id tiebreak; (4) **rail-trails absent from the hiking theme** but real in the **cycling theme** (Virginia Creeper, Mickelson, Elroy-Sparta, Cape Cod ← `cycling.waymarkedtrails.org`). Anything with no traceable centerline — off-trail route-finds (The Maze, Sierra/Wind-River High Routes, Hayduke), canyoneering, the historic auto-tour NHTs — is an **authored approximate corridor**, dashed, never claiming precision (the BDR doctrine). The genuinely-off-trail ones *stay* corridors by design (honest-approximate beats false-precise); thin-but-real OSM (Natchez 7 mi; Ice Age / Hayduke gappy) is kept over a fake-continuous corridor.

**The deepen pass** widened the weird/unique/historic seam via a **6-vein agent fan-out** (slot canyons · vertical/via-ferrata · historic & indigenous routes · urban high-lines & rail-trails · volcanic/cave/badlands · regional long-trail gaps) → +38 journeys + 37 day-activities after dedup, split by the line/point ontology.

**The Wave-1 systematic sweep (the real comprehensiveness move).** Ad-hoc agent brainstorming kept missing whole classes (it missed the Silver Comet, the SC bikepacking routes) — *"if you missed those, you probably missed a lot."* The fix was to stop brainstorming and **enumerate authoritative finite lists** exhaustively, one agent per list: the **USBRS** (numbered US Bicycle Routes), the **Adventure Cycling Route Network**, the **Rails-to-Trails Hall of Fame**, the **National Water Trails System**, the classic **whitewater / Wild&Scenic** canon, the **state long-trail canons**, and the **National Recreation Trails**. Deduped (normalized-name within mode, then cross-mode foot>bike>paddle so a multi-use trail renders once) from ~140 to **432 journeys** — **204 hike / 119 bike / 109 paddle**, all four modes deepened. New `subtype`s fell out: **rail-trail**, **usbr**, **water-trail**. Score-2 filler is *kept* (the explorer's score floor hides it by default — "keeping 2's is fine as long as we have the filters"). The lesson is methodological: for *coverage*, a finite authoritative list beats any amount of generative recall — switch from brainstorming to enumeration the moment you suspect a whole class is missing.

**Trap #5 — same-name wrong-state matches (the sweep's sting).** Adding 100+ new long-trail names multiplied OSM name-collisions: ~41 names "resolved" to a same-named trail in the *wrong* state (West Rim → Zion UT not PA; Manistee River → Alberta, Canada; Mauna Loa → San Bernardino CA not HI) — these pass the coarse US-bounds guard yet land geographically wrong, which is **worse than an honest corridor**. The structural fix lives in `_build/_state_bounds.mjs`: the builder now **resolves a candidate *list* (limit 12), fetches each in rank order, and keeps the first whose centroid falls inside a *declared* state** (`centInState`, +0.7° pad, the same bbox table the post-build `_check_trails` uses). This auto-corrected ~10 by finding the right-state relation deeper in the results; the rest fell through to authored corridors (a Wave-2 agent pass: 24 hike + 7 bike + 28 paddle). Non-deterministic resolvers — Waymarked search occasionally drops the right relation out of the top-N (Kesugi Ridge AK flips between the real trail and a Massachusetts namesake) — get a **pinned corridor** for determinism. All **432 journeys verify centroid-in-declared-state**, headless `_build/verify_trails.mjs` green (counts, per-mode toggles, select→fly, dashed corridors, R1 keystone, no console errors). One agent gotcha worth a guard: corridor-authoring agents intermittently emit `[lng,lat]` (GeoJSON order) instead of the explorer's `[lat,lng]` — always re-validate centroid-in-state after a merge.

**Day-hikes (37 in-park marquee + 123 standalone = 160 activity points)** — the original 37 (slot canyons, via-ferrata, urban high-lines) were geocoded via **Photon** (`_build/geocode_dayhikes.mjs`, state-bbox guard + a hand-filled `COORD` map for the slots Photon can't index); the **Wave-2 standalone sweep** (below) added 123 more. Rendered as **green up-triangles** (distinct from stay-diamonds + camp-circles). The first member of a future "activities" layer (restaurants, viewpoints, swimming holes).

**The Wave-2 standalone sweep — discovery by TYPE (positive enumeration, not negative filter).** The 37 in-park day-hikes are attached to units the atlas already enumerates (Zion, Yosemite). The open question: *which notable day-hikes and trail networks sit on land we DON'T cover* — city/county/regional-district/private/nonprofit/university land, not inside any enumerated park/forest/BLM unit. The **first attempt got the method wrong**: "brainstorm marquee ones regionally, then *subtract* anything on state/federal land." A subtraction has no type behind it, so it left a fuzzy edge — a tail of *borderline survivors* (WA DNR lands, TVA/USACE, Forever Wild) that belonged to no clean sub-layer. The borderline entries were the tell that the **discovery** was wrong, not the entries. The fix: **enumerate each sub-layer positively from its own authoritative handle** (the §13/§15 discipline, finally applied here too). **10 type-agents**, one clean type each:
- **Networks (5):** **IMBA Ride Centers** (the designated Bronze/Silver/Gold list, kept where non-federal) · **private/resort MTB** (Kingdom Trails, Galbraith, Highland, Northstar) · **greenway systems** (Atlanta BeltLine, Razorback, Pinellas) · **city open-space systems** (Boulder OSMP, Portland Forest Park, McDowell Sonoran) · **park/open-space special districts** (the "Forest Preserve District of X County" / "X Metroparks" government roster).
- **Day-hikes (5):** **city/county fitness-peaks** (Camelback, Cowles, Mount Tabor) · **land-trust preserves** (each trust a roster: Trustees of Reservations, TNC, Mohonk, Scenic Hudson) · **university summits** (the "campus mountain" genre — Tumamoc/UA, The Dish/Stanford, Sentinel "M"/U-Montana) · **municipal watershed/utility lands** (Marin MWD, Seattle PU, Denver Water) · **state DNR conservation areas** (WA NRCAs — Mt Si, Mailbox).

Consolidation (`_build/_consolidate_types.mjs`): **per-kind priority dedup** so the more-specific type wins a tie (IMBA > private-MTB; municipal-open-space > special-district — auto-correcting agent over-claims like *Boulder OSMP*); a **guard** that drops federal/state-park leakage incl. **USFS-permit resort bike parks** (Bogus Basin, Trestle — federal land regardless of private operation); cross-kind + existing-37 dedup; centroid-in-declared-state (0 out-of-state). Result: **239 networks + 123 standalone day-hikes** — *more complete* than the regional sweep's 66 + 124, because an authoritative roster (IMBA's list, the special-district government roster, the land-trust org-rosters) surfaces what brainstorming misses. **Networks are a new point primitive** — a labeled *hub* (violet square) at the trailhead, the trail-web supplied by the zoom-gated substrate. The explorer panels **group by discovery-type**, so the sub-layers are legible. The borderline problem dissolves: **state-trust DNR lands are their own sub-layer**, not per-row exceptions; the decision becomes "is this *type* in scope?" (one call per sub-layer) instead of "is this *entry* standalone?" (a judgment per row). The special-district type enumerates exhaustively (129) — kept whole per the **enumerate-comprehensively / publish-by-floor** doctrine (§5a), networks default-OFF + score floor + per-type grouping carrying the tail. **Lesson: when a sweep leaves "borderline" entries, the discovery method is wrong — flip from negative filter to positive enumeration by type, and every entry lands in a named sub-layer with a named source.**

**The comprehensive substrate** — a **zoom-gated Waymarked raster overlay** (`tile.waymarkedtrails.org/hiking/{z}/{x}/{y}.png`), the "every trail" layer that appears past **z≥12** via one `zoomend` handler that **add/removes** the tile layer (not opacity → zero tile fetches when gated out). This is where "thread the layers" becomes spatial — the dense network connects the camps/stays/day-hikes within a park into an explorable environment. cycling/MTB/winter themes are a one-line add once bike/paddle land.

**Render + pipeline:** wired into `ecoregion-explorer-us.html` as the **"on foot"** legend group — **Hiking** (green journey lines, default-ON), **Day-Hikes** (triangles, OFF), **Show all trails** (the substrate) — plus a **"Hikes"** national-panel tab. The `sel` model gained peer `journey`/`dayhike` slots; the R1 keystone holds (toggling never moves the camera). `_build/build_hiking_journeys.mjs` → `data/us/hiking_journeys.json` (2.3 MB, decimated); `_build/geocode_dayhikes.mjs` → `data/us/day_hikes.json`; rosters frozen in `layers/hiking_journeys_meta.json` + `layers/day_hikes_meta.json`.
...
**Standings:** **435 journeys** across three modes — **204 hiking** (126 exact OSM/cycling geometry + 78 corridors) + **119 biking** (48 OSM + 71 corridors) + **112 paddling** (all corridors) — **+ 225 day-hikes** (37 in-park marquee + 188 standalone) **+ 259 standalone trail networks**, joining 6,541 campgrounds + 948 vehicle routes + 1,104 stays + 2,805 activities. All 432 journeys + 123 standalone day-hikes + 239 networks verified centroid-in-declared-state. Committed reproducibility snapshots in `layers/{hike,bike,paddle}_journeys_meta.json` + `layers/{hike,bike,paddle}_corridors.json` (journeys) and `layers/standalone_day_hikes_meta.json` + `layers/networks_meta.json` + `layers/standalone_types_raw.json` (Wave-2, with per-type provenance); the three `build_*_journeys.mjs` read working staging with a committed-layers fallback. The atlas resolves into **four ontological tiers — regions (ecoregions) · routes (drives + journeys, all four modes) · points (camps, stays, day-hikes, network hubs) · environments (the zoom-in substrate)**. The trails-by-mode family (§13 vision) is complete and comprehensive, and the **Wave-2 standalone sweep is DONE — re-discovered by type** (123 day-hikes + 239 networks across 10 sub-layers, headless-verified, dashboard `4eca321`).

**Coverage gap-fill (DONE).** A per-state census found most line layers even or honestly-thin; the one clear real gap was day_hikes (states with famous hikes but 0 entries). Per-state agents filled the six 0-coverage real-gap states with **standalone (non-park)** day hikes — NC/OH/WI/MI/IN/VT, +65 net → **225** (agents correctly skipped the park-land famous ones, which `_consolidate_types`' GUARD auto-drops). Minor top-ups: **networks 239→259** (MA/AL/CT filled with city/county/greenway/land-trust systems) and **paddle 109→112** (PA's Pine Creek Gorge + Allegheny Islands + West Branch Susquehanna, authored corridors). Left honest: book_drives (provenance-bounded — byways already cover scenic drives), dirt_routes (Western), bike (HI/RI islands), prairie/landlocked paddle. Gaps injected into the live by-type pipeline (`_consolidate_types`, NOT the stale parallel `_consolidate_wave2`); dashboard commit `ad531ef` (local). The broader **Activities** point-layer (restaurants, viewpoints, swimming holes…) that day-hikes foreshadowed is now built nationally — see §17.
...
| Day-hikes — in-park marquee (37) | editorial deepen-pass (6-vein agent fan-out) + Photon geocode | point |
| Day-hikes · city/county peaks (53) | the iconic in-city/county-park summit per metro (municipal/county parks-dept land) | point |
| Day-hikes · land-trust preserves (31) | flagship hikeable preserves, enumerated per major land trust (Trustees of Reservations, TNC, Mohonk, Scenic Hudson, Hawk Mountain…) | point |
| Day-hikes · university summits (15) | the "campus mountain" genre — schools that own a hikeable summit (Tumamoc/UA, The Dish/Stanford, Sentinel "M"/U-Montana, Y Mtn/BYU) | point |
| Day-hikes · watershed/utility (12) | municipal water-district / utility recreation lands (Marin MWD, Seattle PU, Denver Water, EBMUD) | point |
| Day-hikes · state conservation areas (12) | state DNR Natural Resources Conservation Areas (WA NRCAs — Mt Si/Mailbox) — its own sub-layer, not a borderline exception | point |
| Networks · IMBA Ride Centers (12) | the IMBA Ride Center designated list (Bronze/Silver/Gold), kept where non-federal | hub point |
| Networks · private/resort MTB (24) | the private/nonprofit/resort MTB canon (Kingdom Trails, Galbraith, Highland, Northstar) | hub point |
| Networks · greenway systems (31) | urban greenway / rail-with-trail network systems (Atlanta BeltLine, Razorback, Pinellas, Bayou Greenways) | hub point |
...
**National rollout — 2,232 things-to-do across all 51 states+DC (DONE):** the right scaling was NOT 50 states × 8 family-agents (that re-discovers the national lists 50× and wastes agents on sparse states). Two axes instead:

- **Axis A — 10 national AUTHORITATIVE-ROSTER agents** (enumerate the whole-US list once, bucket by state — the bounded lists don't vary by region): World Heritage + marquee NHL (153) · AZA zoos & aquariums (99) · flagship museums + observatories (92) · botanical gardens + national public art (104) · Michelin/James-Beard restaurants (132) · theme & water parks (74) · iconic sports venues + marquee races (90) · festivals + state fairs (97) · Atlas Obscura + roadside Americana (97) · iconic experiences + major tours (81). **= 963 after dedup.**
- **Axis B — 12 regional EDITORIAL-MARQUEE agents** (the density-varying tier the national lists miss — local eats, breweries, swimming holes, waterfalls, viewpoints, hot springs, quirky museums, regional oddities, county fairs): Pacific NW 85 · California 114 · Southwest 96 · Mountain West 75 · TX+OK 84 · Great Plains 82 · Great Lakes 95 · South Central 93 · South Atlantic 88 · Mid-Atlantic 102 · New England 80 · HI+AK 58. **= 1,038 after dedup.**

2,071 raw → **2,001 national roster** (cross-framing/coord dedup −70: e.g. the Empire State Building as an NHL vs as an observation-deck experience collapses to one pin) **+ GA 232 = 2,232 served.** Score floor ★3 shows 2,210 (★5×454 · ★4×1045 · ★3×711). The in-state bbox guard held for all but **1 honest drop** (Papahānaumokuākea at −170°, outside the HI box & not a road-trip stop). Agents showed the same positive-enumeration discipline at scale — honest empty subtypes (0 marquee hot-springs in TX/OK/Great-Lakes/New-England; FL natural springs reclassed as swimming-holes; weak play-types confirmed-absent in HI/AK), dropped closed/de-accredited venues, web-verified tricky coords.
...
- **Explorer render polish (DONE).** At ~12k features the dense divIcon layers (activities/stays/day-hikes/networks) now use **Leaflet.markercluster** (camps stay canvas), and the list panels are **viewport-gated** (the "Do" tab lists only what's on screen, ~tens of rows, not ~2,800), re-listing on pan via an R1-safe `moveend`→`renderPanel`. `frame()` stays the sole camera-mover. Headless `verify_render.mjs` 10/10; dashboard `9ec0033` (local).

**We are at a deliberate pause point** — every layer is built nationally AND coverage-audited to an even per-state 80/20 (camps verified comprehensive incl. dispersed/backcountry/BLM/Tribal; activities, stays, day-hikes, networks all gap-filled), and the explorer is render-scale-ready. The standing next move whenever work resumes is **threading the layers** (above).

*Recently done (this pass):* the **Stays coverage audit** (884→1,104, every state ≥9, §14), the **Explorer render-scale polish** (clustering + viewport panels, §18), and the **Trails coverage gap-fill** (day_hikes 160→225, networks 239→259, PA paddle, §15). Before that: the national Activities layer audited to 80/20 (2,805, §17), the national Corps-campground audit (§6 Layer 12), and the Wave-2 standalone trails by type (§15).
