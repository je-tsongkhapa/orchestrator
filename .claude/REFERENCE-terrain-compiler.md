...
**Hiking journeys (92 curated routes)** — `subtype` ∈ National-Scenic / long-distance / park-traverse / historic / weird / one-of-a-kind. The **geometry engine is the Waymarked Trails API** (`{theme}.waymarkedtrails.org/api/v1`, OSM-derived/ODbL): `list/search?query=` → relation id → `details/relation/{id}` carries the full way geometry in **Web Mercator** (decode `lat=(π/2−2·atan(e^(−y/R)))·180/π`, `lon=x/R·180/π`) + `official_length`. Recurse the `route` tree collecting every `LineString`, RDP-simplify (ε=0.0008), emit a multi-line `path` — same schema as the vehicle routes. JMT validated it (<1% length drift, exact High-Sierra bbox).
...
**Bike + paddle (the family completed).** The three human-powered modes merge into one **mode-tagged `JOURNEYS` layer** (hike green / bike blue / paddle teal; per-mode toggles). **Biking (23)** — TransAmerica, Great Divide MTB Route, Pacific Coast, Kokopelli, Monarch Crest, the Adventure-Cycling tiers, Moab/Tahoe MTB — reuses the hiking engine across the Waymarked **cycling + MTB** themes (`{theme}.waymarkedtrails.org`), with the same super-relation-by-bbox-span overrides (TransAmerica = cycling/14469882) and US-bounds guard (Whole Enchilada name-matched a route in *Spain*); US OSM coverage is thin for the Adventure-Cycling coast/tier routes and Moab descents, so those are authored corridors. **Paddling (27)** — Grand Canyon, Middle Fork Salmon, Boundary Waters, Allagash, Buffalo, Rogue — has **no Waymarked theme**, and a river run has no fixed centerline anyway, so every route is an **authored corridor** (rivers traced put-in→take-out following the real bends — agent-authored with verified endpoints; lakes/islands as loop outlines), dashed. The zoom-gated substrate now **stacks the hiking + cycling** Waymarked rasters (foot + bike "every trail").

**Standings:** **142 journeys** across three modes — **92 hiking** (51 exact OSM/cycling geometry + 41 corridors) + **23 biking** (8 OSM + 15 corridors) + **27 paddling** (all corridors) — **+ 37 day-hikes**, joining 6,414 campgrounds + 948 vehicle routes + 884 stays. The atlas resolves into **four ontological tiers — regions (ecoregions) · routes (drives + journeys, all four modes) · points (camps, stays, day-hikes) · environments (the zoom-in substrate)**. The trails-by-mode family (§13 vision) is now complete. Next: thread the layers (proximity join routes↔camps↔stays↔activities into a trip-planner) and the "activities" point-layer beyond day-hikes (restaurants, viewpoints, swimming holes).
