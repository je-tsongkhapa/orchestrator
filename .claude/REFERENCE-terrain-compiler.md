...
**Schema** (one object per route): `name`, `state` (primary) + `states[]`, `tier` (federal / state), `designation` (All-American Road / National Scenic Byway / State Scenic Byway), `subtype` (scenic-drive · bdr · forest-loop · pass · 4x4), `surface`, `length_mi`, `path` (a **multi-line** polyline: an array of `[lat,lng]` segment arrays), `score` 2-5, `why`, `notes`, `layer: "vehicle trails"`.
...
**Pipeline:** `layers/vehicle_trails_meta.json` (committed scores) + live FHWA geometry → `_build/build_byways.mjs` → `data/us/vehicle_trails.json` (gitignored, regenerable) → the explorer fetches it, draws `L.polyline` per route (earthy-brown by score) with **two legend toggles + the shared score floor**: **Scenic Byways** (federal, default-ON) and **State Byways** (state, thinner lines, default-OFF) so 775 routes don't all draw at once. The floor prunes low scores within whichever tier is on — state byways drop 624→489 at the default ≥3.

**Standings:** **775 byways** — 151 federal (120 NSB + 31 AAR) + 624 state — all with exact FHWA geometry, scored & described, **0 provisional**, rendering alongside the 6,414 camps. Combined spread: **82 fives / 247 fours / 305 threes / 141 twos**. **Next:** **book-only** non-designated drives (the *Most Scenic Drives in America* set, via OSRM), and the **dirt sub-modes** — BDRs, forest-road loops, alpine passes, 4x4/OHV (published-GPX ingest).
